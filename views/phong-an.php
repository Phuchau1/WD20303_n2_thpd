...
// Resolve the category by slug 'phong-an'
$category = $categoryModel->getBySlug('phong-an');
if (!$category) {
    http_response_code(404);
    echo "<h1>Danh mục 'Phòng Ăn' không tìm thấy</h1>";
    exit;
}
$category_id = (int)$category['id'];

// Pagination: 10 items per page to make ~20 products across 2 pages
$perPage = 10;
$page = isset($_GET['p']) ? max(1,(int)$_GET['p']) : 1;
$offset = ($page - 1) * $perPage;

$total = $productModel->countByCategory($category_id);
// Limit to exactly 20 products maximum for this room page
$maxTotal = 20;
$displayTotal = min((int)$total, $maxTotal);

// Ensure pagination doesn't point past the limited total
...
<body>
  ...
  <head>
    ...
    <title>Phòng Ăn - Nội Thất Cao Cấp</title>
    <meta name="description" content="Nội thất phòng ăn cao cấp, hiện đại, sang trọng">
    <meta property="og:title" content="Phòng Ăn - Nội Thất Cao Cấp">
    <meta property="og:description" content="Nội thất phòng ăn cao cấp, hiện đại, sang trọng">
    ...
    <section class="section about-room" style="background: linear-gradient(90deg, #f5f5f5 60%, #fbeee6 100%); margin-top: 60px;">
      <div class="container" style="display: flex; flex-wrap: wrap; align-items: center; gap: 40px;">
        <div style="flex:1 1 320px; min-width:260px;">
          <img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=600&h=400&fit=crop" alt="Phòng ăn hiện đại" style="width:100%; border-radius:18px; box-shadow:0 8px 32px rgba(198,164,126,0.13);">
        </div>
        <div style="flex:2 1 400px; min-width:260px;">
          <h2 style="font-size:2.2rem; color:#c6a47e; font-family:var(--font-heading); margin-bottom:18px;">Không Gian Phòng Ăn Hiện Đại</h2>
          <p style="font-size:1.15rem; color:#333; margin-bottom:18px;">Phòng ăn không chỉ là nơi thưởng thức bữa cơm mà còn là không gian gắn kết gia đình, bạn bè. Chúng tôi mang đến các sản phẩm nội thất phòng ăn cao cấp, thiết kế tối giản, hiện đại, phù hợp mọi phong cách sống.</p>
          <ul style="font-size:1rem; color:#555; margin-bottom:18px; line-height:1.8;">
            <li>Chất liệu gỗ tự nhiên, bền đẹp</li>
            <li>Thiết kế tối ưu không gian</li>
            <li>Dễ dàng phối hợp với các không gian khác</li>
